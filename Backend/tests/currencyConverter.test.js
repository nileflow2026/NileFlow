...
describe("applyPsychologicalPricing()", () => {
  ...
  assertEq(
    applyPsychologicalPricing(10000, "KES"),
    9999,
    "10,000 → 9,999 (KES)",
  );
  assertEq(
    applyPsychologicalPricing(50000, "KES"),
    49999,
    "50,000 → 49,999 (KES)",
  );
...
describe("enrichProductWithCurrency()", () => {
  ...
  assert(typeof enriched.price === "object", "price field is now an object");
  ...
  assertEq(
    enriched.price.convertedPrice,
    55000,
    "convertedPrice is 2000 × 27.5 = 55,000",
  );
  assert(
  ...
  assertEq(
    enriched.price.raw,
    2000,
    "raw field preserves original for backward compat",
  );
...
describe("enrichProductsWithCurrency() batch", () => {
  ...
  assertEq(
    enriched[0].price.convertedPrice,
    13750,
    "Product 1: 500 × 27.5 = 13,750",
  );
  assertEq(
    enriched[1].price.convertedPrice,
    27500,
    "Product 2: 1000 × 27.5 = 27,500",
  );
  assertEq(
    enriched[2].price.convertedPrice,
    68750,
    "Product 3: 2500 × 27.5 = 68,750",
  );
});
...
describe("Multi-country scenarios", () => {
  ...
  const ugandaResult = convertPrice({
    amount: 1000,
    from: "KES",
    to: "UGX",
    rate: 27.5,
  ...
  assert(
    ugandaResult >= 27000 && ugandaResult <= 28000,
    `Uganda: UGX ${ugandaResult} (expect ~27,500)`,
  );

  // Kenya user (no conversion needed)
  const kenyaResult = convertPrice({
    amount: 1000,
    from: "KES",
    to: "KES",
    rate: 1,
